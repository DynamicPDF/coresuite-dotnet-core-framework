...
namespace coresuite_asp_dotnet_framework_mvc_cs.CodeFiles
{
    ...
    {
        internal static byte[] Run(W9FormModel w9FormData)
        {
            try
            {
                // Create a document and set it's properties
                PdfDocument pdf = new PdfDocument(HttpContext.Current.Server.MapPath("~/PDFs/fw9AcroForm_18.pdf"));
        ...
        internal static byte[] RunFormFill(W9FormModel w9FormData)
        {
            // Create a MergeOptions object to import the AcroForm
            MergeOptions options = new MergeOptions(false);

            // Create a document and set it's properties
            MergeDocument document = new MergeDocument(HttpContext.Current.Server.MapPath("~/PDFs/fw9_18.pdf"), options);
            document.Creator = "FormFill";
